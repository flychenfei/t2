...
public class GithubRepositoriesHandler {
	 ...
	 * @param name name of repository
	 * @param login current github user login name
	 * @param pageIndex Issues page number
	 * @param pageSize every page number
	 * @return
	 * @throws IOException
	 */
	...
	public WebResponse getIssues(@WebUser User user,@WebParam("name") String name,
		...
		repo.setName(name);
		try{
			return WebResponse.success(githubRepositoriesService.getIssues(repo, user, state, Integer.parseInt(pageIndex), Integer.parseInt(pageSize))).set("result_count", githubRepositoriesService.countIssues(repo, user,state));
		}catch(Exception e){
			return WebResponse.fail(e.getMessage());
		}
	}
	 ...
	 * @param name name of repository
	 * @param login current github user login name
	 * @param state Issues state
	 * @return
	 * @throws IOException
	 */
	@WebGet("/github/getIssuesCount")
	public WebResponse getIssuesCount(@WebUser User user,@WebParam("name") String name,
								 @WebParam("login") String login,@WebParam("state") String state
								 ) throws IOException {
		Repository repo = new Repository();
		org.eclipse.egit.github.core.User owner = githubUserService.getGithubUser(user);
		...
		repo.setName(name);
		try{
			return WebResponse.success(githubRepositoriesService.countIssues(repo, user,state));
		}catch(Exception e){
			return WebResponse.fail(e.getMessage());
		}
	}
